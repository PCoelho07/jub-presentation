...
function Playlist() {
  ...
  useEffect(() => {
    const fetchSongs = async () => {
      try {
        setLoading(true);
        const {
          data: { playlist_name, songs },
        } = await api.get(`/playlists/${id}/songs`);
        setPlaylist({
          songs: songs.map((row) => ({
            id: row.id,
            name: row.name,
  ...
  const chooseLinkTo = (song) => {
    return song.hasGuide ? `/song/${song.id}` : `/song/create-guide/${song.id}`;
  };
    ...
    <Container>
      <Flex alignItems="baseline">
        <Heading fontSize={"lg"} mb="4">
          {loading ? <Skeleton height="7" width="28" /> : playlist?.name}
        </Heading>
      ...
      <EditPlaylist isOpen={isOpen} onClose={onClose} playlist={playlist} />
    </Container>
  );
}
      ...
      <HStack>
        {song.hasGuide && (
          <Button
            ...
            as={Link}
            to={`/song/${song.id}`}
            rightIcon={<MdOutlineMap size="20px" />}
          >
            Ver mapa
          </Button>
        )}
        (
        <Button size="sm" as={Link} to={`/song/create-guide/${song.id}`}>
          Editar
        </Button>
        )
      </HStack>
